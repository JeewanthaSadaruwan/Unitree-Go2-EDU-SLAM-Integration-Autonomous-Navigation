...
# Kill static TF publisher
pkill -f "static_transform_publisher.*base_link.*base_footprint" && echo "✓ Killed static_transform_publisher" || echo "  (static_transform_publisher not running)"
...
REMAINING=$(ps aux | grep -E "slam_toolbox|odom_to_tf|pointcloud_to_laserscan|static_transform_publisher.*base" | grep -v grep | wc -l)

if [ $REMAINING -eq 0 ]; then
    echo ""
    echo "✓ All SLAM processes stopped successfully!"
    echo ""
    echo "You can now start fresh with: ./go2_slam_improved.sh"
else
    echo ""
