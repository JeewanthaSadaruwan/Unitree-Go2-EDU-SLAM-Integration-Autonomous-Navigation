...
class GoalPoseRelay(Node):
    ...
    def __init__(self):
        ...
        self._sub = self.create_subscription(PoseStamped, '/goal_pose', self._on_goal, 10)
        self._last_goal_handle = None

        self.get_logger().info('goal_pose_relay started, listening on /goal_pose')

    def _on_goal(self, msg: PoseStamped) -> None:
        if msg.header.frame_id == '':
            self.get_logger().warn('Ignoring /goal_pose without frame_id')
            return
